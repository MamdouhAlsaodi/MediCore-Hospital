...
| 11 — Idempotent synthetic demo data | Done | `016b56a` (#6) | `DemoDataInitializerTest` (4); `MEDICORE_DEMO_SEED` opt-in |
...
- **Duplicate MRN may surface as 500, not 400/409.** Uniqueness is enforced only by the database unique constraint on `patients.medical_record_number`; `PatientService.create` has no pre-check and `GlobalExceptionHandler` has no integrity-violation handler. The frontend handles failure generically. Fixing it needs its own backend packet.
- **Demo-seeded rows carry no audit events.** The opt-in seeder writes through repositories, bypassing the services that own audit. Intentional and documented in `docs/runbook.md`; do not read the Audit screen as a complete history of a seeded database.
...
- **Docs drift observed during the accuracy pass (outside this packet's allowed paths, flagged for the supervisor):** the `frontend/src/authorization.js` header comment still describes the pre-Task-9 policy gaps as open even though `SecurityConfig` (commit `b49e549`) now enforces the aligned matrix; `docs/security.md` still mentions a "checked-in JWT secret and development admin" that `cd2ba13` removed.
