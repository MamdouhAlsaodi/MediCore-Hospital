...
One complete, test-pinned hospital workflow on synthetic data only:
...
6. **Audit evidence** — every successful mutation produces an ADMIN-visible audit event.

Role-based access is enforced in the backend (`401` for no session, `403` for a refused role), independent of what the interface hides. Failure states — validation, malformed references, permission denials, session expiry — are handled and tested at every step. The full walk-through with real request/response shapes: [`docs/patient-journey.md`](docs/patient-journey.md).
...
- **Tooling:** JUnit integration suites (32 backend tests), Vitest + Testing Library (61 frontend tests), a repeatable API smoke script with per-step timings.
...
Optional synthetic demo cohort (3 patients, 2 professionals, 2 appointments; idempotent; off by default): start the backend with `MEDICORE_DEMO_SEED=true`. Never enable it against a shared database.
...
| Document | Contents |
|---|---|
| [`docs/patient-journey.md`](docs/patient-journey.md) | The demonstrated workflow end-to-end: steps, roles, API shapes, failure states, audit evidence |
| [`docs/architecture/patient-journey.md`](docs/architecture/patient-journey.md) | Architecture diagram and layer boundaries (UI → adapters → controllers → services → repositories/audit), RBAC layers, error contract |
...
This repository is intentionally **not** production-ready: no Docker, no Pilot preparation, no browser-verified screenshots, no regulatory review, no backups/observability/integrations, and no clinical certification. Docker is postponed by design, and the next step after this milestone is a formal Training/Portfolio acceptance review — not deployment. Details: [`docs/implementation-status.md`](docs/implementation-status.md).
