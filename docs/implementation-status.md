...
- **Docs drift observed during the Task 13 accuracy pass — mostly closed by MEDICORE-STALE-DOCS-GAPFIX-025:** the `frontend/src/authorization.js` header now describes the enforced Task 9 matrix, `docs/security.md` now describes the env-only secrets model (the "checked-in JWT secret and development admin" wording removed at `cd2ba13` is gone), and `docs/patient-journey.md`, `docs/implementation-status.md`, and `docs/api.md` record the 409 contract. Still stale, outside the allowed paths (flagged for the supervisor): the "DOCUMENTED POLICY MISMATCH" comment inside `frontend/src/authorization.test.js` (comment only — its assertions pin the current map) and the duplicate-MRN paragraph in `docs/architecture/patient-journey.md`.
