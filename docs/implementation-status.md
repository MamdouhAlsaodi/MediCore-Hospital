...
# Phase 4 — production-like resilience status (specs/004-production-like-resilience)

Scope reminder: everything below is **Training/Portfolio engineering evidence on synthetic data** — a demonstration of realistic practice, not a production, clinical, compliance, capacity, or SLA claim. Phase 3's "explicitly NOT done" list above was written at the Phase 3 stop gate; Phase 4 deliberately implements the previously postponed items inside that same boundary (Docker/Compose, PostgreSQL migrations, backup/restore, observability), while everything still listed as not done there (Pilot, real data, SaaS tenancy, integrations, payment, MFA/SSO, clinical semantics) **remains not done**.

## Phase 4 fresh verification evidence (at this documentation revision)

Canonical command: `scripts/phase4/acceptance.sh` (fail-fast; 13 serial stages; exit 0 only when every stage exits 0; missing Docker/psql/node/maven capability exits 2 BLOCKED, proven negatively by stage 0 on every run).

- Backend: `mvn test` (JDK 21, Maven 3.9) — see totals below; includes real Testcontainers PostgreSQL migration, context, scope, concurrency, typed-values, observability, security-matrix, OpenAPI-contract, and rate-limit suites.
- Frontend: `npm install` + `npm test` (Vitest) + `npm run typecheck` + `npm run build` — totals below.
- OpenAPI drift: `scripts/phase4/check-openapi-drift.sh` — served contract regenerated byte-identical vs `api/openapi/medicore-v1.yaml` and `frontend/src/generated/api/`.
- Containers: `check-container-static.sh` static guard; Compose review stack built and proven healthy with service/port ownership; full Playwright E2E (desktop + mobile journeys, context discrimination, backend restart with zero pending migrations, PostgreSQL stop/restore recovery, exact non-2xx matrix).
- Backup/restore: `test-backup-restore.sh` — archive + verified checksum + fresh-target restore + allowlisted invariants identical, plus seven fail-closed refusals.
- Security: `check-security-headers.sh` (static + live same-origin probes) on top of the backend CORS/header/actuator/auth matrix suites.
- Observability: `observability-live-rehearsal.sh` — live liveness/readiness split under real DB loss and recovery on the same process; JSON log structure; bounded metric labels; no sensitive canaries in logs/metrics.
- Public safety: `check-public-artifacts.sh` clean; `git diff --check` clean; changed paths confined to the packet allowlist.
- CI policy: **local-only** — no `.github/` directory exists at baseline, so no repository CI policy is demonstrated; none is claimed or fabricated.

Terminal fresh run (2026-09-16): **PASS** — `scripts/phase4/acceptance.sh` exited 0 after all 13 serial stages passed in 1124s on HEAD `a76c9da`. Backend: 237/237 with zero failures/errors/skips. Frontend: 251/251 across 16 files plus strict typecheck and production build. Backup/restore: 15/15 assertions. Observability: live DB-loss/readiness split and same-process recovery passed under Temurin JDK 21.0.12. Playwright container journey: 6 passed, 2 intentionally skipped mobile duplicates; migrations applied=4/failed=0/shipped=4; teardown removed the stack, network, volume, and released loopback ports. OpenAPI drift, public-artifact scan, security headers, docs/traceability, and final disposable cleanup all passed.

## Phase 4 known limits (honest)

- **Single-host, single-run evidence.** Concurrency, restart, and dependency-loss proofs run on one developer workstation against disposable containers; they demonstrate transactional and recovery semantics, not distributed or production behavior.
- **Forward-fix migrations.** Versioned migrations are forward-only; rollback is by restore from a verified backup archive, not by down-migrations.
- **Uniqueness stays global** for MRN and invoice numbers (explicit FR-014 decision, documented in `docs/pdr.md` Phase 4 addendum).
- **Local-only acceptance.** The canonical gate runs on the developer host; no CI execution evidence exists or is claimed.
- **Rate limiting and headers are training-grade.** The login rate limiter and header set are tested and bounded but are not a substitute for a hardened production edge.
- **Everything in the Phase 3 "Explicitly NOT done" list not covered by Phase 4's declared scope remains not done.**

## Phase 4 stop gate

Phase 4 closes with the terminal canonical acceptance run and the FR-001..020 / SC-001..010 evidence map in `docs/evidence/phase4-verification.md`. A terminal PASS authorizes exactly the synthetic Training/Portfolio demonstration scope — no Pilot, no real data, no clinical use, no production/capacity/SLA/compliance claim, and no deployment or release without the owner's explicit, separate decision.
