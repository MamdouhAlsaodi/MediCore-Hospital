...
// Deterministic EOF convention: every generated file ends with EXACTLY one
// newline. Without this, an operation block whose queryInterface section is
// empty ends the file with a trailing blank line, and the regenerated client
// drifts from its committed form on the openapi-drift check.
const normalizeEof = (contents) => `${contents.replace(/\n+$/, '')}\n`;
...
writeFileSync(join(outDir, 'schemas.ts'), normalizeEof(schemasTs));
writeFileSync(join(outDir, 'operations.ts'), normalizeEof(operationsTs));
writeFileSync(join(outDir, 'index.ts'), normalizeEof(indexTs));
